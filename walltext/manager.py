...
class WalltextManager:
    ...
    def _build_list_mode(self, parent: tk.Frame) -> None:
        ...
        editor_buttons = tk.Frame(right, bg=UI_BG)
        editor_buttons.pack(fill="x", padx=12, pady=12)
        self._button(editor_buttons, "➕ Add", self.add_item).pack(side="left", padx=(0, 8))
        self._button(editor_buttons, "✍ Update", self.update_selected).pack(side="left", padx=(0, 8))
        self._button(editor_buttons, "⎚ Clear", self.clear_editor).pack(side="left", padx=(0, 8))
        self._button(editor_buttons, "📝 Edit Md", self.open_selected_in_markdown_mode).pack(side="left", padx=(0, 8))
        self._button(editor_buttons, "▶ Apply Selected", self.apply_selected).pack(side="left", padx=(0, 8))
        self._button(editor_buttons, "▶ Apply Next", self.apply_now).pack(side="left")
